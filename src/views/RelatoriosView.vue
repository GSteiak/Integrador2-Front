<template>
  <div class="p-d-flex p-jc-center p-ai-center" style="min-height: 100vh; padding: 20px">
    <Card style="width: 80vw; max-width: 1000px">
      <template #title> Relatórios de Agendamentos </template>
      <template #content>
        <Button
        ...
        <DataTable
          :value="agendamentosRelatorio"
          :paginator="true"
          :rows="10"
          responsiveLayout="scroll"
          v-if="agendamentosRelatorio.length > 0"
        >
        ...
        <p v-else-if="reportGenerated" class="p-mt-4">
...
const loadMedicosOptions = async () => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: 'Falha ao carregar lista de médicos',
      life: 3000,
    })
  }
}
...
const generateReport = async () => {
  ...
  if (!filters.value.dataInicio || !filters.value.dataFim) {
    toast.add({
      severity: 'warn',
      summary: 'Atenção',
      detail: 'Selecione a Data Início e a Data Fim.',
      life: 3000,
    })
    return
  }
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: 'Falha ao gerar relatório de agendamentos.',
      life: 5000,
    })
  }
}
