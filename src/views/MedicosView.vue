  ...
  <div class="p-d-flex p-jc-center p-ai-center" style="min-height: 100vh; padding: 20px;">
    <Card style="width: 80vw; max-width: 1000px;">
      ...
      <template #content>
        ...
        <Button label="Adicionar Novo Médico" icon="pi pi-plus" class="p-button-success" style="margin-top: 1em; margin-bottom: 2em;" @click="openNew" />
        ...
        <DataTable :value="medicos" :paginator="true" :rows="10" responsiveLayout="scroll">
            ...
            <Column header="Ações">
                <template #body="slotProps">
                    <Button icon="pi pi-pencil" class="p-button-rounded p-button-success p-mr-2" @click="editMedico(slotProps.data)" />
                    <Button icon="pi pi-trash" class="p-button-rounded p-button-warning" @click="confirmDeleteMedico(slotProps.data)" />
                </template>
            </Column>
        </DataTable>

        <Dialog v-model:visible="medicoDialog" :style="{width: '450px'}" header="Detalhes do Médico" :modal="true" class="p-fluid">
            <div class="p-field">
                <label for="nome">Nome</label>
                <InputText id="nome" v-model.trim="medico.nome" required="true" autofocus :class="{'p-invalid': submitted && !medico.nome}" />
                <small class="p-invalid" v-if="submitted && !medico.nome">Nome é obrigatório.</small>
            </div>
            <div class="p-field">
                <label for="especialidade">Especialidade</label>
                <InputText id="especialidade" v-model.trim="medico.especialidade" required="true" :class="{'p-invalid': submitted && !medico.especialidade}" />
                <small class="p-invalid" v-if="submitted && !medico.especialidade">Especialidade é obrigatória.</small>
            </div>
            <div class="p-field">
                <label for="crm">CRM</label>
                <InputText id="crm" v-model.trim="medico.crm" required="true" :class="{'p-invalid': submitted && !medico.crm}" />
        ...
        <ConfirmDialog></ConfirmDialog> <Toast /> </template>
    </Card>
  </div>
</template>
...
const loadMedicos = async () => {
  try {
    ...
    toast.add({severity:'error', summary: 'Erro', detail:'Falha ao carregar médicos', life: 3000});
  }
};
...
const saveMedico = async () => {
    ...
    if (medico.value.nome && medico.value.especialidade && medico.value.crm) {
        try {
            if (medico.value.id) {
                const response = await axios.put(`http://localhost:8080/api/medicos/${medico.value.id}`, medico.value);
                console.log('Médico atualizado:', response.data);
                toast.add({severity:'success', summary: 'Sucesso', detail:'Médico atualizado', life: 3000});
            } else {
            ...
            toast.add({severity:'error', summary: 'Erro', detail:'Falha ao salvar médico', life: 3000});
        }
    }
};
    ...
    confirm.require({
        ...
        accept: async () => {
            ...
            try {
                ...
                toast.add({severity:'success', summary: 'Sucesso', detail:'Médico excluído', life: 3000});
                await loadMedicos(); // Recarrega a lista
            } catch (error) {
                console.error('Erro ao excluir médico:', error);
                toast.add({severity:'error', summary: 'Erro', detail:'Falha ao excluir médico', life: 3000});
            }
        },
        reject: () => {
