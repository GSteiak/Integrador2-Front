<template>
  <div class="p-d-flex p-jc-center p-ai-center" style="min-height: 100vh; padding: 20px">
    <Card style="width: 80vw; max-width: 1000px">
      <template #title> Gerenciar Pacientes </template>
      <template #content>
        <Button
          ...
          <Column header="Ações">
            <template #body="slotProps">
              ...
              <Button
                ...
                @click="confirmDeletePaciente(slotProps.data)"
              />
            </template>
          </Column>
        </DataTable>
        ...
        >
          <div class="p-field">
            ...
            <InputText
              ...
              required="true"
              autofocus
              :class="{ 'p-invalid': submitted && !paciente.nome }"
            />
          ...
          <div class="p-field">
            ...
            />
            <small class="p-invalid" v-if="submitted && !paciente.dataNascimento"
          ...
          <div class="p-field">
            ...
            <InputText
              id="telefone"
              v-model.trim="paciente.telefone"
              :class="{ 'p-invalid': submitted && !paciente.telefone }"
            />
            <small class="p-invalid" v-if="submitted && !paciente.telefone"
          ...
          <div class="p-field">
            ...
            <InputText
              id="email"
              v-model.trim="paciente.email"
              :class="{ 'p-invalid': submitted && !paciente.email }"
            />
            <small class="p-invalid" v-if="submitted && !paciente.email"
...
const savePaciente = async () => {
  ...
  if (
    ...
    paciente.value.telefone &&
    paciente.value.email
  ) {
    try {
      const dataParaAPI =
        paciente.value.dataNascimento instanceof Date
      ...
      if (paciente.value.id) {
        const response = await axios.put(
          `http://localhost:8080/api/pacientes/${paciente.value.id}`,
          pacienteParaSalvar,
        )
      ...
      toast.add({
        severity: 'error',
        summary: 'Erro',
        detail: 'Falha ao salvar paciente',
        life: 3000,
      })
    }
  }
    ...
    accept: async () => {
      try {
        ...
        toast.add({
          severity: 'error',
          summary: 'Erro',
          detail: 'Falha ao excluir paciente',
          life: 3000,
        })
      }
    },
