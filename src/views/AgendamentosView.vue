<template>
  <div class="p-d-flex p-jc-center p-ai-center" style="min-height: 100vh; padding: 20px">
    <Card style="width: 80vw; max-width: 1000px">
      <template #title> Gerenciar Agendamentos </template>
      <template #content>
        <Button
        ...
        <Button
          label="Adicionar Novo Agendamento"
          icon="pi pi-plus"
          ...
          <Column header="Ações">
            <template #body="slotProps">
              <Button
                ...
                @click="editAgendamento(slotProps.data)"
              />
              <Button
                icon="pi pi-trash"
                class="p-button-rounded p-button-warning"
                @click="confirmDeleteAgendamento(slotProps.data)"
              />
            </template>
          </Column>
        </DataTable>
        ...
        <Dialog
          v-model:visible="agendamentoDialog"
          :style="{ width: '450px' }"
        ...
        >
          <div class="p-field">
            ...
            />
            <small class="p-invalid" v-if="submitted && !agendamento.data"
          ...
          <div class="p-field">
            ...
            />
            <small class="p-invalid" v-if="submitted && !agendamento.hora"
              >Hora é obrigatória.</small
            >
          </div>
          <div class="p-field">
            ...
            <InputText
              id="tipoConsulta"
              v-model.trim="agendamento.tipoConsulta"
              required="true"
              :class="{ 'p-invalid': submitted && !agendamento.tipoConsulta }"
            />
            <small class="p-invalid" v-if="submitted && !agendamento.tipoConsulta"
              >Tipo de Consulta é obrigatório.</small
            >
          </div>
          ...
          <div class="p-field">
            ...
            />
            <small class="p-invalid" v-if="submitted && !agendamento.medico"
              >Médico é obrigatório.</small
            >
          </div>
          ...
          <div class="p-field">
            ...
            />
            <small class="p-invalid" v-if="submitted && !agendamento.paciente"
...
const loadAgendamentos = async () => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: 'Falha ao carregar agendamentos',
      life: 3000,
    })
  }
}
...
const loadMedicosAndPacientes = async () => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: 'Falha ao carregar listas de médicos e pacientes',
      life: 3000,
    })
  }
}
...
const saveAgendamento = async () => {
  ...
  if (
    ...
    agendamento.value.hora &&
    agendamento.value.tipoConsulta &&
    agendamento.value.medico &&
    agendamento.value.paciente
  ) {
    try {
      // Formata a data para AAAA-MM-DD
      const dataParaAPI =
        agendamento.value.data instanceof Date
      ...
      const horaParaAPI =
        agendamento.value.hora instanceof Date
      ...
      if (agendamento.value.id) {
        ...
        const response = await axios.put(
          `http://localhost:8080/api/agendamentos/${agendamento.value.id}`,
      ...
      toast.add({
        severity: 'error',
        summary: 'Erro',
        detail: 'Falha ao salvar agendamento',
        life: 3000,
      })
    }
  }
    ...
    accept: async () => {
      try {
        ...
        toast.add({
          severity: 'error',
          summary: 'Erro',
          detail: 'Falha ao excluir agendamento',
          life: 3000,
        })
      }
    },
